...
/**
 * Generate registry navigation JSON file
 * @param payload Payload CMS instance
 * @returns Promise<void>
 */
export const generateRegistryNavigation = async (payload: Payload): Promise<void> => {
  console.log('Generating registry navigation JSON...');
  ...
  try {
    ...
    if (existingCacheEntry.docs.length > 0) {
      ...
      console.log(`Registry navigation updated in MongoDB cache (ID: ${existingCacheEntry.docs[0].id})`);
    } else {
      // Create new entry
      ...
      console.log(`Registry navigation saved to MongoDB cache (ID: ${createdCache.id})`);
    }

  } catch (error) {
    console.error('Error generating registry navigation:', error);
    throw error;
  }
};
